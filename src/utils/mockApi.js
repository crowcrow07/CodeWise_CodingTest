class MockApi {
  ...
  async delete({ mailUidList = [] || 0 }) {
    ...
    try {
      const tempArray = Array.isArray(mailUidList)
        ? mailUidList
        : [mailUidList];
      const tempData = this.#db.articles.filter(
        (v) => !tempArray.includes(v.mailUid)
      );
      if (tempData.length === this.#db.articles.length) {
        result.status = 500;
        throw new Error(
          `Cannot read property mailUid ${tempArray.join(", ")} of undefined`
        );
      }
  ...
  async get(
    { mailType, mailTitle, ismailIUse, limit, currentPage } = {} || undefined
  ) {
    const result = {
      data: null,
      status: null,
    ...
    try {
      const tempDB = this.#db.articles.filter((value) =>
